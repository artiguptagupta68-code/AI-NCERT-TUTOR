...
import gdown  # safer for Google Drive files

# ----------------------------
# CONFIG
# ----------------------------
# Use relative paths inside app folder (Streamlit Cloud safe)
ZIP_DRIVE_ID = "1g1qnn03XSxamjz33ocnwYcZOuJCNW8jj"  # Google Drive file ID
ZIP_PATH = "ncrt_subject.zip"       # relative path
EXTRACT_DIR = "ncert_extracted"    # relative folder inside app
CHUNK_SIZE = 800
CHUNK_OVERLAP = 150
...
if not os.path.exists(ZIP_PATH) or os.path.getsize(ZIP_PATH) == 0:
    ...
    gdown.download(
        url=f"https://drive.google.com/uc?id={ZIP_DRIVE_ID}",
        output=ZIP_PATH,
        quiet=False
    )
...
try:
    ...
    raise Exception(f"The file {ZIP_PATH} is not a valid ZIP file. Make sure the Google Drive link points to a proper ZIP.")
